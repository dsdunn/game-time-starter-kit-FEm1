...
class Game {
  ...
  highScoreLocalStorage() {
    this.highScore.push(this.score)
    let stringifiedHighScore = JSON.stringify(this.highScore)
    
    localStorage.setItem('highScoreArray', stringifiedHighScore)
  }
  updateHighScores() {
    let parsedHighScore = JSON.parse(localStorage.getItem('highScoreArray'))
